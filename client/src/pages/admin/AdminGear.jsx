...
export default function AdminGear() {
  ...
  const fetchItems = async (currentPage, searchStr = '') => {
    setLoading(true);
    try {
      // Filter exclusively by category=gear
      const res = await axiosInstance.get(`/motorcycles?category=gear&page=${currentPage}&limit=${limit}&search=${encodeURIComponent(searchStr)}`);
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black tracking-tight">Gear & Apparel</h1>
          </div>
          <p className="text-dark-muted mt-1">View, edit, and manage all gear and apparel inventory.</p>
        </div>
        <Link to="/admin/gear/new" className="flex items-center gap-2 px-6 py-3 bg-accent-primary text-dark-bg font-bold rounded-xl hover:bg-accent-secondary transition-all shadow-lg shrink-0">
          <FiPlus /> Add Item
        </Link>
      </motion.div>
                  ...
                  <tr key={item._id} className="hover:bg-dark-bg/30 transition-colors group">
                    ...
                    <td className="p-5">
                      <div className="flex justify-end gap-2">
                        <Link to={`/admin/gear/${item._id}/edit`} className="w-10 h-10 rounded-xl bg-dark-bg flex items-center justify-center text-dark-muted hover:text-accent-primary hover:bg-accent-primary/10 transition-all">
                          <FiEdit2 size={18} />
                        </Link>
                  ...
                  <tr>
                    <td colSpan="6" className="p-10 text-center text-dark-muted">
                      <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <p>No gear found. Add one to get started.</p>
                      </div>
                    </td>
                  </tr>
                )}
