...
export default function Products() {
  ...
  const brands = ['Harley-Davidson', 'Kawasaki', 'Yamaha', 'BMW', 'Honda', 'Ducati', 'Royal Enfield', 'Triumph', 'KTM', 'Suzuki', 'Aprilia'];
          ...
          >
            <div>
              <h1 className="text-4xl md:text-5xl font-bold mb-2">
                Explore Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent-primary to-accent-secondary">Collection</span>
              </h1>
              <p className="text-dark-muted text-lg">
                {filters.category && (
                  <span className="text-accent-primary mr-2">{formatCategory(filters.category)} ·</span>
                )}
                {pagination.total ? `${pagination.total} Premium Motorcycles Available` : 'Find your dream ride'}
              </p>
            </div>
          ...
          `}>
            ...
            >
              ...
              <div className="space-y-8">
                ...
                <div>
                  <label className="block text-xs uppercase tracking-wider text-dark-muted font-semibold mb-3">Category</label>
                  <div className="space-y-2">
                    {CATEGORIES.map((cat) => (
                      <label key={cat.value || 'all'} className="flex items-center gap-3 cursor-pointer group p-2 rounded-lg hover:bg-dark-bg/50 transition-colors">
                        <div className="relative flex items-center">
                          <input
                            ...
                            className="peer appearance-none w-4 h-4 border-2 border-dark-border rounded-full checked:border-accent-primary checked:bg-accent-primary transition-all"
                          />
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center border-2 border-dashed border-dark-border rounded-xl bg-dark-card/30">
                ...
                <p className="text-dark-muted max-w-md mx-auto mb-6">
                  We couldn&apos;t find any motorcycles matching your current filters.
                </p>
                <button
                  onClick={clearFilters}
